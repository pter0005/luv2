...
export default function PreviewContent({
    ...
    return (
        <div className="relative w-full h-full flex items-center justify-center">
            <div className={cn(
                "relative w-full max-w-md h-[85vh] bg-zinc-800 rounded-[2.5rem] p-3 border-4 border-zinc-700 shadow-2xl shadow-primary/20 flex flex-col overflow-hidden"
            )}>
                 <motion.div 
                                    ...
                                    {formData.galleryImages.map((image: any, index: number) => (
                                        <SwiperSlide key={image.path || index} className="bg-transparent">
                                            <div className="relative w-full aspect-square">
                                                <Image src={image.url} alt={`Pré-visualização da imagem ${index + 1}`} fill className="object-cover" unoptimized sizes="(max-width: 768px) 100vw, 33vw"/>
                                            </div>
                                        </SwiperSlide>
                                    ))}
                                </Swiper>
