...
const Countdown = ({ targetDate, style = 'Padrão', color = '#FFFFFF' }: CountdownProps) => {
  ...
  if (style === 'Simples') {
    return (
        <div className="w-full max-w-2xl mx-auto text-center p-4 rounded-lg bg-black/30 backdrop-blur-sm">
            <p className="text-md md:text-lg" style={{ color: color }}>
                <span className="text-muted-foreground">Compartilhando momentos há</span>{' '}
                ...
                <span className="font-bold">{String(timeLeft.days).padStart(2, '0')}</span> dias{' '}
  ...
  return (
    <div className="w-full mx-auto">
        ...
        <div className="grid grid-cols-6 gap-1 md:gap-2 text-center">
          {timeUnits.map(unit => (
            <div key={unit.label} className="p-1 md:p-2 bg-white/5 rounded-lg backdrop-blur-sm border border-white/10 shadow-lg">
