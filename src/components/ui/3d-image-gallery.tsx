...
function FloatingCard({
  ...
  const imageSize = useMemo(() => {
    const cardWidth = isMobile ? 4.8 : 5.5;
    ...
    >
      ...
      >
        <div
          ...
          style={{
            width: `${imageSize.width * (isMobile ? 32 : 40)}px`,
            boxShadow: hovered
        ...
        >
            ...
            <p className={`text-white font-semibold leading-tight line-clamp-2 mt-2 ${isMobile ? 'text-[10px]' : 'text-sm'}`}>{card.title}</p>
             {card.date && (
              <p className={`text-primary/90 font-bold mt-1 tracking-wide ${isMobile ? 'text-[10px]' : 'text-xs'}`}>
