...
import { FullScreenCardView, type Card } from "./timeline-mobile"
...
/* Placeholder — shown during Suspense load OR if a card throws */
function CardMeshFallback({ position, cardH }: { position: { x: number; y: number; z: number }; cardH: number }) {
  ...
  useFrame(({ camera, clock }) => {
    ...
    if (matRef.current) matRef.current.opacity = 0.22 + 0.1 * Math.sin(clock.elapsedTime * 2 + pulse.current)
  })
...
/* Per-card error boundary — bad image fails silently to placeholder instead of crashing whole scene */
...
function GalleryCard({ card, position, cardH, onClick, phase }: { card: Card; position: { x: number; y: number; z: number }; cardH: number; onClick: () => void; phase: number }) {
  const fallback = <CardMeshFallback position={position} cardH={cardH} />
  return (
  ...
  return (
    <>
      ...
      <OrbitControls
        ...
        autoRotate
        autoRotateSpeed={isMobile ? 0.5 : 0.35}
      ...
      <div className="absolute top-0 left-0 w-full px-4 pt-4 pb-20 z-10 flex justify-between items-start bg-gradient-to-b from-black/75 to-transparent pointer-events-none">
        <div className="text-white">
          ...
          <p className="text-[10px] text-white/50 mt-0.5">
            {isMobile ? "Arraste · Pinça para zoom · Toque para ver" : "Arraste · Scroll para zoom · Clique para ver"}
          </p>
        </div>
        <button
          onClick={onClose}
