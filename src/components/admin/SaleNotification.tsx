...
// Cash register sound as base64 — short "ka-ching!" effect
...
export function SaleNotification() {
  ...
  // Auto-subscribe admin browser for push notifications (runs once)
  useEffect(() => {
    if (typeof window === 'undefined') return;
    if (localStorage.getItem(ADMIN_PUSH_KEY)) return; // already subscribed
  ...
  useEffect(() => {
    ...
    // Only listen for sales created after now (ignore historical)
    ...
    const unsub = onChildAdded(salesRef, (snap) => {
      if (!initializedRef.current) {
        ...
        return; // skip first callback which may fire for existing data
      }
    ...
    // After a short delay, mark as initialized (skip any initial data)
